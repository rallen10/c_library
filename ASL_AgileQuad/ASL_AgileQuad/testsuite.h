...
static void mavlink_test_obs_repel_force_ned(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
	mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
	...
	mavlink_obs_repel_force_ned_t packet1, packet2;
        ...
        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_obs_repel_force_ned_encode(system_id, component_id, &msg, &packet1);
...
static void mavlink_test_ASL_AgileQuad(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
	...
	mavlink_test_obs_repel_force_ned(system_id, component_id, last_msg);
}
